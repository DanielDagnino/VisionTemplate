...
from vision.dataset.ds_clf import DatasetClf
from vision.losses_metrics import get_loss, get_metric
from vision.models.helpers import save_checkpoint, load_checkpoint
from vision.models.seg_models_pytorch import SegModelPytorchClf, SegModelPytorchClfV2
...
from vision.trainer.trnr_clf import trainer
...
def main_worker(this_gpu, n_gpus_per_node, cfg):
    ...
    if cfg.engine.model.name == SegModelPytorchClf.__name__:
        logger.info(f"{SegModelPytorchClf.__name__} building")
        model = SegModelPytorchClf(**cfg.engine.model.args)
    elif cfg.engine.model.name == SegModelPytorchClfV2.__name__:
        logger.info(f"{SegModelPytorchClfV2.__name__} building")
        model = SegModelPytorchClfV2(**cfg.engine.model.args)
    else:
    ...
    for stage in stages_trainer_sorted:
        ...
        if cfg.dataset.name == DatasetClf.__name__:
            dataset = DatasetClf(stage=stage, **cfg.dataset.get(stage), rank=cfg.rank)
        else:
